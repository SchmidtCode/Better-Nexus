...
| `A.JournalAssociationReadModel` | One defensive slot/candidate/association materialization for the visible Journal | JournalTab/tests | GA/SV -> bounded read model | nil when slots unavailable; exact and ambiguity rules remain authoritative | A-W/A-X |
...
| `A.AuthorityUnresolved` | Pending Select lacks exact bounded granted authority, including Store-loss session quarantine | AutomationRuntime | GA/AUT -> boolean | true on unusable Store until explicit run/reset; false only with usable Store and no row/quarantine | A-I/A-X |
...
| `A.JournalAssociationRevisions` | Allocation-free slot/Wishlist/association/active-slot generations with pending Echo reconciliation | JournalTab/tests | GA semantic generations -> - | nil tuple when reconciliation or revision evidence is unavailable | A-C/A-X |
...
| Store pre-init access, root/subtable identity, ordered owners, future/unknown fields, dependency failure, retry, and per-character unresolved Select ownership | `tests/run_store_contract_characterization.lua`, `tests/run_store_additive_migrations.lua`, `tests/run_recorded_picks_authority.lua` | Store is the sole `NexusDB` binder; evidence initializes before catalog and compaction, later owners never run after an earlier failure, and unresolved Select authority is isolated under the exact current character without consuming obsolete account-root data. |
...
| Defensive reads, association identity, dirty-set consumption, retry limits, in-flight spacing, external-action consumption, and exact granted Select confirmation | `tests/run_gameadapter_contract_characterization.lua`, `tests/run_snapshot_wishlist_association.lua`, `tests/run_echo_catalog_revision.lua`, `tests/run_recorded_picks_authority.lua` | GameAdapter owns captured service state and returns defensive projections where specified; Select authority survives reload/timeout and clears only for the current owner after settled exact granted-generation evidence, while every observed contradictory generation remains terminal even before settlement. |
| Ordered `ToggleTomeEcho`, `SelectPerk`, `BanishPerk`, `RequestReroll`, `FreezePerk`, `ActivateServerBuildSlot`, and `SaveServerBuildSlot` calls | `tests/run_gameadapter_contract_characterization.lua`, `tests/run_integration.lua`, `tests/run_recorded_picks_authority.lua` | Gameplay writes stay behind GameAdapter validation, spacing, in-flight, retry, and unresolved Select authority safeguards; manual Select uses the same mutation barrier, including post-deadline and overlapping-intent quarantine. |
